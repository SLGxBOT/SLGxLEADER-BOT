...
//---------------------------------------------------------------------------
smd({pattern: "dalle",alias : ['dall','dall-e'],desc: "chat with an AI",category: "ai", use: '<Hii, SLG xTEAM Info>',filename: __filename,},
    async(Aviator, msg,text) =>{
  if ( Config.OPENAI_API_KEY=='' ||  !Config.OPENAI_API_KEY.startsWith('sk') ) return msg.reply('```You Dont Have OPENAI API KEY \nPlease Create OPEN API KEY from Given Link \nhttps://platform.openai.com/account/api-keys\nAnd Set Key in Heroku OPENAI_API_KEY Var```')
...
///=============================================
smd({pattern: "uptime",alias: ["runtime"],desc: "Tells runtime/uptime of bot.",category: "misc", filename: __filename},async(SLGxLEADER, msg, text) => {const upt = runtime(process.uptime());msg.reply(`*_Uptime of ${Config.botname}: ${upt}_*`);})
//---------------------------------------------------------------------------
//                  RREPOSITORY  COMMAND
//---------------------------------------------------------------------------
smd({
    ...
    async(SLGxLEADER, citel) => {
      try{
          let { data } = await axios.get('https://api.github.com/repos/SLGxBOT/SLGxLEADER-BOT')
        let cap = `
        
🔰 *SLGxLEADER-BOT Repo Info* 🔰
  ...
  *❲❒❳ Repo:* _${github}_
  *❲❒❳ Scan:* _${scan}_\n
  *❲❒❳ Visit For Tutorial* _https://www.Youtube.com/@SLGLEADER_
${Config.caption?'\n\n'+Config.caption : ''}`.trim();
	
        try{ return await SLGxLEADER.bot.sendMessage(citel.chat,  {   image: { url: await botpic() },    caption: cap,  } , {   quoted: citel, })}catch{return await citel.send(cap),{},'',citel}
...
smd({
  ...
  *❲❒❳ Support:* _${gurl}_ 
${Config.caption?'\n\n'+Config.caption : ''}`.trim();
       try{ return await SLGxLEADER.bot.sendMessage(citel.chat, { image: { url: await botpic() },caption:ter, },{ quoted: citel, }); }catch{citel.send(cap),{},'',citel}
