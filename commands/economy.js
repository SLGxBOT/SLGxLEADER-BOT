...
smd({ pattern: "daily", desc: "daily gold.", category: "economy",filename: __filename, /* react: "💷" */ },
   ...
   const daily  = await eco.daily(msg.sender, "SLGxLEADER", 2000); //give 500 for daily, can be changed
...
smd({
    ...
    async(Aviator, msg, text,{ isCreator }) => {
       ...
       const balance  = await eco.balance(users, "SLGxLEADER")
       await eco.deduct(users, "SLGxLEADER", balance.wallet);
...
async(Aviator, msg, text,{ isCreator }) => {
   ...
   const balance  = await eco.balance(user, "SLGxLEADER")
   switch (value) {
       case '1000':
       case '1':
       if (k > balance.wallet ) return msg.reply(`*_You need to pay 🪙100 to increase bank capacity ~ 1000 sp_*`);
         const deduct1 = await eco.deduct(user, "SLGxLEADER", 100);
         const add1 = eco.giveCapacity(user,"SLGxLEADER", 1000);
       ...
       if (k < balance.wallet) return msg.reply(`*You need to pay 🪙1000 to increase bank capacity ~ 100000 sp*`);
         const deduct2 = await eco.deduct(user,"SLGxLEADER", 1000);
         const add2 = eco.giveCapacity(user, "SLGxLEADER", 100000);
       ...
       if (k < balance.wallet) return msg.reply(`You need to pay 🪙10000 to increase bank capacity ~ 1000 sp`);
          const deduct3 = await eco.deduct(user, "SLGxLEADER", 10000);
          const add3 = eco.giveCapacity(user, "SLGxLEADER", 10000000);
...
await msg.reply('*What are you trying to do📉*.')
   ...
   async(Aviator, msg, text,{ isCreator }) => {
     ...
     //  let users = msg.mentionedJid ? msg.mentionedJid[0] : msg.msg.contextInfo.participant || false;
       ...
       let d = parseInt(text)
       const deposit = await eco.deposit(msg.sender, "SLGxLEADER", d);
       const balance = await eco.balance(msg.sender, "SLGxLEADER")
   ...
   async(Aviator, msg, text,{ isCreator }) => {
   let h = await eco.lb("SLGxLEADER",10);
...
async(Aviator, msg, text,{ isCreator }) => {
   ...
   if(!user) return msg.reply('Please give me any user🤦‍♂️.');
       ...
       const balance = await eco.balance(user1, "SLGxLEADER");
       let a = (balance.wallet) < parseInt(word)
       //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
       if(a == true) return msg.reply("you dont have sufficient money to transfer👎");

       const deduct = await eco.deduct(user1, "SLGxLEADER", value[0]);
       const give = await eco.give(user2, "SLGxLEADER", value[0]);
  ...
  // return await Aviator.bot.sendButtonText(msg.chat, `*📠 Transaction successful of ${value[0]} 💰*`, `${Config.ownername.split(' ')[0]}-Economy Version: 0.0.6`, msg);
   ...
   async(Aviator, msg, text,{ isCreator }) => {
       ...
       let mongoschemas = zerogroup.economy || "false";
       if (mongoschemas == "false") return msg.reply("*🚦Economy* is not active in current group.");
        const balance = await eco.balance(msg.sender,"SLGxLEADER"); //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
   ...
   async(Aviator, msg, text,{ isCreator }) => {
       if(!isCreator) return
        ...
        await eco.give(users, "SLGxLEADER", parseInt(text.split(' ')[0]));
   ...
   if(!users) return msg.reply('Please give me user to rob.')
       ...
       const user2 = users
       const k = 1000
       const balance1  = await eco.balance(user1, "SLGxLEADER")
   const balance2  = await eco.balance(user2, "SLGxLEADER")
   ...
   switch (random) {
     ...
     const deduff = Math.floor(Math.random() * 1000)	    
         await eco.deduct(user2, "SLGxLEADER", deduff);
         await eco.give(msg.sender, "SLGxLEADER", deduff);
       ...
       case 'caught':
          ...
          await eco.deduct(user1, "SLGxLEADER", rmoney);
...
///.mentionedJid[0] ? m.mentionedJid[0] : m.sender
    ...
    const twice = gg*2
         var hjkl;
        if(opp==='left') {   hjkl = 'https://github.com/SLGxBOT/POORNAxSLG/blob/main/POORNA/STICKER2/leftr.webp?raw=true'   } 
   else if(opp==='right') {  hjkl = 'https://github.com/SLGxBOT/POORNAxSLG/blob/main/POORNA/STICKER2/rightr.webp?raw=true'  } 
   else if(opp==='up') {     hjkl = 'https://github.com/SLGxBOT/POORNAxSLG/blob/main/POORNA/STICKER2/upr.webp?raw=true'     }
   else if (opp==='down'){  hjkl = 'https://github.com/SLGxBOT/POORNAxSLG/blob/main/POORNA/STICKER2/downr.webp?raw=true'    }
   else{   msg.reply(`Please provide direction(left,right,up,down).\nEg:- ${prefix}gamble 200 left`)  }
  let media = await getBuffer(hjkl)
  msg.reply(media,{packname:"SLGxLEADER",author:'Economy'},"sticker")
    ...
    if (!text) return msg.reply(`Example:  ${prefix}gamble 100 direction(left,right,up,down)`);
          ...
          if ( r == opp){
          let give = await eco.give(user , "SLGxLEADER", twice);    //msg.react('⭐️')
   ...
   //return await Aviator.bot.sendButtonText(msg.chat, `*📈 You won 🪙${twice}*`, `${Config.ownername.split(' ')[0]}-Economy \n Version: 0.0.6`, msg);
                 ...
                 let deduct = await eco.deduct(user, "SLGxLEADER", texts[0]);
...
if(value<=balance.wallet){
   ...
   if ((f1 !== f2) && f2 !== f3){
       const deduct1 = await eco.deduct(user, "SLGxLEADER", deduff);
       return msg.reply(`${mess1}\n\n*Big Lose -->* _🪙${deduff}_`)
    }
    else if ((f1 == f2) && f2 == f3){
       const give1 = await eco.give(user, "SLGxLEADER", deduff/2);
       return msg.reply(`${mess2}\n*_Little Jackpot -->* _🪙${deduff/2}_`)
    }
    else if ((f1 == f2) && f2 !== f3){
       const give2 = await eco.give(user, "SLGxLEADER", deduff);
       return msg.reply(`${mess3}\n*Small Win -->* _🪙${deduff}_`)
    }
    else if ((f1 !== f2) && f1 == f3){
       const deduct2 = await eco.deduct(user, "SLGxLEADER", deduff);
       return msg.reply(`${mess5}\n\n*Small Lose -->* _🪙${deduff}_`)
    }
    else if ((f1 !== f2) && f2 == f3){
       const give4 = eco.give(user,"SLGxLEADER", deduff);
       return msg.reply(`${mess3}\n\n*Small Win -->* _🪙${deduff}_`)
    }
    else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
       const give5 = eco.give(user,"SLGxLEADER", deduff*20);
   ...
   return msg.reply('You don\'t have enough 💰amount in your👛 wallet.\n- Please don\'t provide 🤑amount.')
           ...
           if ((f1 !== f2) && f2 !== f3){
              const deduct1 = await eco.deduct(user, "SLGxLEADER", 50);
                     msg.reply(`${mess1}\n\n*Big Lose -->* _🪙50_`)
           }
           else if ((f1 == f2) && f2 == f3){
              const give1 = await eco.give(user,"SLGxLEADER", 100);
                    msg.reply(`${mess2}\n*_Little Jackpot -->* _🪙100_`)
           }
           else if ((f1 == f2) && f2 !== f3){
              const give2 = await eco.give(user, "SLGxLEADER", 20);
                    msg.reply(`${mess3}\n*Small Win -->* _🪙20_`)
           }
           else if ((f1 !== f2) && f1 == f3){
              const deduct2 = await eco.deduct(user, "SLGxLEADER", 20);
                    msg.reply(`${mess5}\n\n*Small Lose -->* _🪙20_`)
           }
           else if ((f1 !== f2) && f2 == f3){
              const give4 = eco.give(user, "SLGxLEADER", 20);
                    msg.reply(`${mess3}\n\n*Small Win -->* _🪙20_`)
           }
           else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
              const give5 = eco.give(user, "SLGxLEADER", 1000);
                   msg.reply(`${mess4}\n\n_🎊 JackPot --> _🪙1000_`)
           }
...
async(Aviator, msg, text,{ isCreator }) => {
   ...
   if (mongoschemas == "false") return msg.reply("*🚦Economy* is not active in current group.");
   const kg = 100
           const balance1  = await eco.balance(msg.sender, "SLGxLEADER")
   ...
   if(lol){
       ...
       const give2 = await eco.give(msg.sender, "SLGxLEADER", deduff*2);
  ...
  // return await Aviator.bot.sendButtonText(msg.chat,str+`You got ${deduff*10} in your wallet.`, `${Config.ownername.split(' ')[0]}-Economy \n Version: 0.0.6`, msg);
   ...
   const deduct1 = await eco.deduct(msg.sender, "SLGxLEADER", deduff);
   let st = `\n🎰 Slot Machine Result\n     ${i}\n\n      ${j}\n\n      ${k}\n\nNot Jacpot📉 but lost `
           let str = st.replace(/1/g, `🔴`).replace(/2/g, `🔵`).replace(/3/g, `🟣`).replace(/4/g, `🟢`).replace(/5/g, `🟡`).replace(/6/g, `⚪️`).replace(/7/g, `⚫️`).replace(/:/g, `    `)
