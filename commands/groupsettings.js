...
smd({
        ...
        async(SLGxLEADER.bot, msg, text , {isCreator}) => {
            if (!msg.isGroup) return msg.reply(tlang().group);
          let check = text ? text : '';
...
smd({
 ...
 async(SLGxLEADER, msg, text,{ isCreator }) => {
      ...
      let metadata = await SLGxLEADER.bot.groupMetadata(msg.chat);
      var ppuser;
      let num = msg.sender;
      ...
      try {  ppuser = await SLGxLEADER.bot.profilePictureUrl(num, 'image') }catch { ppuser = 'https://i.imgur.com/FfBybSI.jpg' ; }
      return await SLGxLEADER.bot.sendMessage(msg.chat, { image: { url: ppuser }, caption: welcome_messages,mentions: [num] } )
 ...
 async(SLGxLEADER, msg, text,{ isCreator }) => {
    ...
    try {  ppuser = await SLGxLEADER.bot.profilePictureUrl(num, 'image') }catch { ppuser = 'https://i.imgur.com/FfBybSI.jpg' ; }
        ...
        return await SLGxLEADER.bot.sendMessage(msg.chat, { image: { url: ppuser }, caption: goodbye_messages, mentions: [num] })
...
async(SLGxLEADER, msg, text,{ isCreator }) => {
    if (!msg.isGroup) return msg.reply(tlang().group);
    const groupMetadata = msg.isGroup ? await SLGxLEADER.bot.groupMetadata(msg.chat).catch((e) => {}) : "";
...
if(ty){  
    ...
    async(SLGxLEADER, msg, text) => {
        var bio = await SLGxLEADER.bot.fetchStatus(msg.sender) || {};
        var bioo = bio.status ;
        let meh = msg.sender;
...
let pfp;
        try {pfp = await SLGxLEADER.bot.profilePictureUrl(msg.sender, "image");} 
...
`;
        ...
        return await SLGxLEADER.bot.sendMessage(msg.chat, buttonMessage, { quoted: msg });
...
userq.level + 1
        ...
        try { ppuser = await SLGxLEADER.bot.profilePictureUrl(msg.sender, "image");}
        catch { ppuser = THUMB_IMAGE;}
        return await SLGxLEADER.bot.sendMessage(msg.chat, { image:{url: ppuser } , caption: textr, }, { quoted: msg, });
    }
)
//---------------------------------------------------------------------------
smd({
...
//---------------------------------------------------------------------------
smd({ on: "text" }, async(SLGxLEADER, msg) => {
    ...
    if (hasLeveledUp){
        ...
        {
            await SLGxLEADER.bot.sendMessage(msg.chat, { image: {  url: await botpic() },
        caption: `
╔════◇
║ *Wow,Someone just*
║ *leveled Up huh⭐*
