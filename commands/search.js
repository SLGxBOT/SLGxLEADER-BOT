...
//   *Release :* _${data.release_date}
   ...
   let buttonMessaged = {
               ...
               await SLGxLEADER.bot.sendMessage(citel.chat, buttonMessaged, { quoted: citel, });
       }
    )
    //------------------------------------------------------------------------------------
smd({
...
console.log("text : " , textt)
    ...
    //return await SLGxLEADER.bot.sendMessage(citel.chat ,{image : media } , {quoted:citel} )
 }
...
smd({
        ...
        async(SLGxLEADER, citel, text) => {
            ...
            SLGxLEADER.bot.sendMessage(citel.chat, {  image: { url: fids.data.Poster, }, caption: imdbt,  }, {   quoted: citel,  });
...
smd({
        ...
        async(SLGxLEADER, citel, text) => {
            ...
            SLGxLEADER.bot.sendMessage( citel.chat, {  text: textw }, {  quoted: citel } );
...
for (let url of urlsArray) { SLGxLEADER.bot.sendMessage(citel.chat , {image : {url : url} } )  }
} 
...
smd({
 ...
 if(!text) return await citel.reply('Give Me Number without + sign. Example: .iswa 9231844741xx')
        ...
        for (let i = 0; i < randomxx; i++) {
            ...
            var anu = await SLGxLEADER.bot.onWhatsApp(`${number0}${i}${number1}@s.whatsapp.net`);
            var anuu = anu.length !== 0 ? anu : false
            try 
            {
                  try { var anu1 = await SLGxLEADER.bot.fetchStatus(anu[0].jid); } 
                  catch { var anu1 = '401' ; }
...
smd({
    ...
    async(SLGxLEADER, citel, text) => {
if(!text) return await citel.reply('Give Me Number without + sign. Example: .nowa 947426571XX')
const inputNumber = text.split(" ")[0]
if (!inputNumber.includes('x')) return citel.reply(`*You did not add x in number.*\nExample: ${prefix}nowa 947426571XX  \n ${Config.caption}`)
...
for (let i = 0; i < randomxx; i++) 
{
    ...
    const anu = await SLGxLEADER.bot.onWhatsApp(`${number0}${i}${number1}`);
    const anuu = anu.length !== 0 ? anu : false;
    try 
    {
         const anu1 = await SLGxLEADER.bot.fetchStatus(anu[0].jid);
