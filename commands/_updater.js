...
//---------------------------------------------------------------------------
     ...
     smd({
        ...
        async(SLGxLEADER, msg, text,{ isCreator }) => {
                ...
                await msg.send(" *SLGxBOT Updater Started...!*\n\n*Please wait you have new updates*\n *───────────────────────────*\n"+update +"\n\n\n"+Config.caption);
                await sleep(3000);
          try{
...
smd({   pattern: "restart", desc: "To restart bot",category: "tools", filename: __filename }, async(SLGxLEADER, msg,text,{ isCreator }) => {  if (!isCreator) return msg.reply(tlang().owner);  const { exec } = require("child_process"); msg.reply('Restarting'); exec('pm2 restart all'); });
