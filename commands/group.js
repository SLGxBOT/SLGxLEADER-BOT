...
smd({
    ...
    async(SLGxLEADER.bot, msg, text) => {
        msg.reply(`*Check your Pm ${tlang().greet}*`);
        await SLGxLEADER.bot.sendMessage(`${msg.sender}`, {
            image: log0,
...
async(SLGxLEADER, msg, text,{ isCreator }) => {
    ...
    try {
        await SLGxLEADER.bot.groupUpdateDescription(msg.chat, text);
        msg.reply('*_✅Group description Updated Successfuly.!_*') 
        return await SLGxLEADER.bot.sendMessage(msg.chat, { react: { text: '✨', key: msg.key }});
    } catch(e) { return await SLGxLEADER.bot.sendMessage(users , {text :"Error While Updating Group Description\nReason : " + e, } ,{quoted : msg})   }
}
)
//---------------------------------------------------------------------------
smd({
...
async(SLGxLEADER, msg, text,{ isCreator }) => {
    ...
    try {
        await SLGxLEADER.bot.groupUpdateSubject(msg.chat, text)
        msg.reply('*_✅Group Name Updated Successfuly.!_*') 
        return await SLGxLEADER.bot.sendMessage(msg.chat, { react: { text: '✨', key: msg.key }});
    } catch(e) { return await SLGxLEADER.bot.sendMessage(users , {text :"_Error While Updating Group Name_\nReason : " + e, } ,{quoted : msg})   }
}
)
    //---------------------------------------------------------------------------
...
smd({
    ...
    async(SLGxLEADER, msg, text,{isCreator}) => {
        ...
        await SLGxLEADER.bot.updateBlockStatus(users, "block")
            .then((res) => { return SLGxLEADER.bot.sendMessage(msg.chat, { react: { text: '✨', key: msg.key }});    })		    //console.log(jsonformat(res))
...
smd({
	    ...
	    let num = users.replace("@s.whatsapp.net","")
            await SLGxLEADER.bot.updateBlockStatus(users, "unblock")
...
var str1 = await SLGxLEADER.bot.groupInviteCode(msg.chat)
...
var code = await SLGxLEADER.bot.groupRevokeInvite(msg.chat)
...
➲ *Message :* ${text ? text : "blank Message 𝙎𝙇𝙂 𝙭𝙏𝙀𝘼𝙈 "} \n ${Config.caption} \n\n
➲ *Author:* ${msg.pushName} 🔖
`
        for (let mem of participants) { textt += `📍 @${mem.id.split("@")[0]}\n`;   }
        SLGxLEADER.bot.sendMessage(msg.chat, { text: textt,  mentions: participants.map((a) => a.id) }, {  quoted: msg });
    }
)
	...
	for (let i of users) { 
		...
		{ 
			...
			try { await SLGxLEADER.bot.groupParticipantsUpdate(msg.chat, [i.id], "remove"); hmanykik++ ;  }
...
smd({
    ...
    msg.sender.split("@")[0]
            ...
            for (let i of owner) {
                SLGxLEADER.bot.sendMessage(i + "@s.whatsapp.net", {
                    text: textt + teks1,
                    mentions: [msg.sender],
                }, {
                    quoted: msg,
...
smd({
        ...
        async(SLGxLEADER.bot, msg, text) => {
            ...
            if (/viewOnce/.test(mime)) {
                ...
                await SLGxLEADER.bot.relayMessage(msg.chat, prep.message, { messageId: prep.key.id });
            } else {
    ...
    smd({
        ...
        async(SLGxLEADER, msg, text,{ isCreator }) => {
            ...
            await SLGxLEADER.bot.sendMessage(msg.chat, { poll: { name: poll,  values: options } })
        }
    )
    ...
    smd({
        ...
        async(SLGxLEADER, msg, text ,{ isCreator }) => {	
            ...
            try {
                ...
                await SLGxLEADER.bot.groupParticipantsUpdate(msg.chat, [users], "promote");
                await msg.send(`*User promoted Succesfully!*`)
            } catch(e) {
                console.log("Promote error : " , e )
                await SLGxLEADER.bot.sendMessage(msg.chat, { react: { text: '❌', key: msg.key }});
...
//if (!isCreator) return msg.reply("```Only My Owner Can Use This Command```")
    ...
    try {
        let users = msg.quoted ? msg.quoted.sender : msg.mentionedJid[0] ? msg.mentionedJid[0] : text.replace(/[^0-9]/g, "") + "@s.whatsapp.net";
        if (!users) return await msg.send("*_Uhh Dear, Reply/Mention to an User_*");
        await SLGxLEADER.bot.groupParticipantsUpdate(msg.chat, [users], "demote");
...
smd({
	...
	//if (!isCreator) return msg.reply("*_Only My Owner Can Use This Command_*")
            ...
            try {
                ...
                await SLGxLEADER.bot.groupParticipantsUpdate(msg.chat, [users], "remove");
    ...
    smd({
        ...
        async(SLGxLEADER, msg, text) => {
            ...
            if (action.startsWith("close") || action.startsWith("mute") ) {
                await SLGxLEADER.bot.groupSettingUpdate(msg.chat, "announcement").then((res) => msg.reply(`*_Group Chat Muted!!!_*`)).catch((err) => msg.error(err));
            } else if (text.toLowerCase().startsWith("open")||text.toLowerCase().startsWith("unmute") ){
                await SLGxLEADER.bot.groupSettingUpdate(msg.chat, "not_announcement").then((res) => msg.reply(`*_Group Chat Unmuted!!!_*`)).catch((err) => msg.error(err));
            }
else if( action.startsWith("detail") || action.startsWith("info") ){
try{
    const pp = await SLGxLEADER.bot.profilePictureUrl(msg.chat, 'image').catch(_ => THUMB_IMAGE) || THUMB_IMAGE;
...
`.trim()
 ...
 try{ await SLGxLEADER.bot.sendMessage(msg.chat,{image:{url : pp ? pp : THUMB_IMAGE } , caption: ginfos } , {quoted:msg }) }catch(e){ return await msg.send(ginfos,{},"",msg),console.log("error in group info,\n"  , e)   }
    ...
    //  let buttons = [{ buttonId: `${prefix}group open`, buttonText: { displayText: "📍Unmute",},type: 1,},{buttonId: `${prefix}group close`,buttonText: {displayText: "📍Mute",},type: 1, },];     await SLGxLEADER.bot.sendButtonText(msg.chat,buttons,`Group Mode`, SLGxLEADER.bot.user.name, msg);
...
/*
            ...
            let media = await SLGxLEADER.bot.downloadAndSaveMediaMessage(msg.quoted);
            await SLGxLEADER.bot.updateProfilePicture(msg.chat, {
                    url: media,
                })
...
└───────────
`.trim()
return await SLGxLEADER.bot.sendMessage(msg.chat,{text : tag ,mentions: [msg.sender, ...groupAdmins.map(v => v.id) ,]} ,)
    ...
    smd({
        ...
        async(SLGxLEADER, msg, text,{isCreator,botNumber}) => {
	//if (!isCreator) return msg.reply("```Only My Owner Can Use This Command```")
            if (!msg.isGroup) return msg.reply(tlang().group);
            const groupAdmins = await getAdmin(SLGxLEADER.bot, msg)
            const isBotAdmins = msg.isGroup ? groupAdmins.includes(botNumber) : false;
            ...
            if(msg.sender == botNumber ){
                await SLGxLEADER.bot.groupParticipantsUpdate(msg.chat, [users], "add");
                await msg.send(`*User Added Succesfully!*`)
                return await SLGxLEADER.bot.sendMessage(msg.chat, { react: { text: '✨', key: msg.key }});
            }else {
                await SLGxLEADER.bot.sendMessage(msg.chat, { react: { text: '❌', key: msg.key }});
                await SLGxLEADER.bot.sendMessage(users , {text : `Here's The Group Invite Link\n User @${msg.sender.split("@")[0]} Wants To Add You in bellow Group\n https://chat.whatsapp.com/${await SLGxLEADER.bot.groupInviteCode(msg.chat)} _ \n ------------ Join If YOu Feel Free?` ,mentions:[msg.sender,]} , {quoted : msg })
    ...
    smd({
        ...
        async(SLGxLEADER, msg, text,{ isCreator }) => {
            if (!isCreator) return msg.reply(tlang().owner)
            n = await SLGxLEADER.bot.groupFetchAllParticipating();
...
let getGroups = await SLGxLEADER.bot.groupFetchAllParticipating();
...
await Promise.all(anu.map(async i => {
  let metadata = await SLGxLEADER.bot.groupMetadata(i);
  await sleep(2000); 
 res += ` ------------- ${i} -------------\n`;
...
return await msg.reply(res);
	//return await SLGxLEADER.bot.sendMessage(msg.chat,{text:res},{quoted:msg})
        ...
        async(SLGxLEADER, msg, text,{ isCreator, botNumber }) => {
          ...
          try{
            if ((msg.quoted.isBot && botNumber === msg.quoted.sender ) || ( botNumber === msg.quoted.sender && isCreator )  ) {
                ...
                return await SLGxLEADER.bot.sendMessage(msg.chat, { delete: key })
            }
            if (!msg.quoted.isBot ) {
                if (!msg.isGroup) return msg.reply(tlang().group)
                const groupAdmins = await getAdmin(SLGxLEADER.bot, msg)
...
let contextInfo = {forwardingScore: 999,isForwarded: true, externalAdReply: { title:'SLGxLEADER-Md Broadcast',body:msg.pushName,renderLargerThumbnail: true,thumbnail: log0,mediaType: 1,mediaUrl: '',sourceUrl: gurl,showAdAttribution: true,},}
