...
///============================================================================================
smd({ pattern: "ptv", desc: "send ptv Message of video", category: "whatsapp",filename: __filename},async(SLGxLEADER, msg, text,{cmdName , isCreator}) => {
   ...
   return await forwardMessage(msg.chat, SLGxLEADER, msg, cmdName )
})////------------------------------------------------------------
smd({pattern: "#", desc: "Save whatsapp status",category: "whatsapp",filename: __filename},async(SLGxLEADER, msg, text,{cmdName , isCreator}) => {
   if(!msg.quoted) return await msg.send("*Uhh Please, reply to whatsapp status*")
   let SLGxBOT_num = await SLGxLEADER.bot.decodeJid(msg.user)
   return await forwardMessage(SLGxBOT_num, SLGxLEADER, msg, cmdName )
})//--------------------------------------------------------------------
smd({pattern: "save",desc: "Save Message to log number",category: "whatsapp",filename: __filename},async(SLGxLEADER, msg, text,{cmdName , isCreator}) => {
   if(!isCreator) return await msg.send(tlang().owner)
   if(!msg.quoted) return await msg.send("*Uhh Please, reply to a Message*")
   let SLGxBOT_num = await SLGxLEADER.bot.decodeJid(msg.user)
   return await forwardMessage(SLGxBOT_num, SLGxLEADER, msg, cmdName )
})///================================================================================
smd({ on: "text" }, async (SLGxLEADER,msg,text)=> {
   if(msg.quoted && msg.text.toLowerCase().includes("send") ){
       let SLGxBOT_num = await SLGxLEADER.bot.decodeJid(msg.user)
       if(msg.quoted.sender === SLGxBOT_num && msg.quoted.chat === 'status@broadcast' ){ return await forwardMessage(msg.chat, SLGxLEADER.bot, msg, 'send' ); }
   }
})//==================================================================
...
async(SLGxLEADER, msg,text,{cmdName , isCreator}) => {
   ...
   const SLGxBOT_num = await SLGxLEADER.bot.decodeJid(msg.user); return await updateProfilePicture(SLGxLEADER , SLGxBOT_num , msg , 'pp' );
 })
